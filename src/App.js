  ...
  transferAmount = async () => {
    ...
    if (scatter) {
      if (this.state.from && this.state.to) {
        ...
        try {
          let amount1 =
            (this.state.amount === "" ? "0" : this.state.amount) + ".0000 EOS";
  ...
  createAccount = async () => {
    ...
    if (this.state.to && this.state.from && this.state.amount) {
      if (this.state.from.length !== 12) {
          ...
          try {
            ...
            if (account.length > 0) {
              ...
              const transactionObject = {
                from: sendFrom,
                to: paymentDetails.data.SEND_TO,
                value: web3.utils.toWei(paymentDetails.data.ETH, 'ether'),
              };
                const sendTransaction = await web3.eth.sendTransaction(transactionObject);
                console.log('sendTransaction => ', sendTransaction);
                const obj = {
                  name: this.state.from,
                  ownerKey: this.state.to,
                  activeKey: this.state.amount,
                  currency: 'ETH',
                ...
                const message = createAccount.data;
        ...
        // let config = {
        //   chainId:
        //     "038f4b0fc8ff18a4f0842a8f0564611f6e96e8535901dd45e43ac8691a1c4dca", // 32 byte (64 char) hex string
      ...
      <div className="App">
        <div className="container">
          ...
          <div style={{ display: this.state.create }}>
            ...
            <button onClick={this.createAccount}>Sign With MetaMask</button>
          </div>
        </div>
      </div>
    );
